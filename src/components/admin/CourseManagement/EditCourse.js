  ...
  const loadCourse = async () => {
    try {
      ...
      await courseDetails.course.filter(course => course._id == params.id).map(data => {
        ...
        setFieldValues({
          ...fieldValues, "id": params.id, courseName: data.courseName,
          author: data.author,
  ...
  return (
    ...
    <Form.Control type="text" placeholder="Enter email"value={courseDetailObject.courseName} />
        ...
        <Form.Group className="mb-3" controlId="formBasicEmail">
          ...
          <Form.Control type="text" placeholder="Enter email" value={fieldValues.actualPrice} onChange={handleChange('actualPrice')} />
