...
function ManageReferalAgents() {
  ...
  const addNewAgent = async () => {
    try {
      let { data } = await axios.post(
        process.env.REACT_APP_SERVER + "/admin/addNewAgent",
        { fieldValues },
        {
          headers: {
  ...
  const loadAgents = async () => {
    let { data } = await axios.get(
      process.env.REACT_APP_SERVER + "/admin/getAllAgents",
      {
        headers: {
          authorization: "AdminJwt " + adminDetails.adminJwt,
        },
      ...
      <table class="table" id="userTable">
        <thead>
          <tr>
            ...
            <th scope="col">Users course purchased</th>
          </tr>
        </thead>
        <tbody>
          {agents.map((data, i) => {
            return (
              <>
                <tr>
                  ...
                  <td>/signup?referral={data._id}</td>
                  <td>{data.usersJoined}</td>
                </tr>
              </>
            );
          })}
